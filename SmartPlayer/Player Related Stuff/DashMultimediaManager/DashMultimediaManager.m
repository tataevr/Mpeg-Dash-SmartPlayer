...
static NSString * const TemplateNumber            = @"$Number$";
static NSString * const TemplateRepresentationID  = @"$RepresentationID$";
static NSString * const TemplateTime              = @"$Time$";

@interface DashMultimediaManager () <MpdManagerDelegate> {
    double              _audioTimeScale;
    double              _videoTimeScale;
    
    NSTimeInterval      _totalMediaDuration;
    
    AudioData *       _firstAudioData;
    VideoData *       _firstVideoData;
...
@implementation DashMultimediaManager

+ (void) __attribute__((noreturn)) dashMultimediaThreadEntryPoint:(id)__unused object {
    do {
        @autoreleasepool {
            [[NSThread currentThread] setName:@"DashMultimediaManagerThread"];
...
- (void)setDelegate:(id<DashMultimediaMangerDelegate>)delegate {
    _delegate = delegate;
...
- (void)mpdManager:(MpdManager *)manager didFinishParsingMpdFile:(MPD *)mpd {
    ...
    if (_firstStart){
        if (_streamType == StreamTypeDynamic) {
            [self dynamic_processFirstStart];
        } else if (_streamType == StreamTypeStatic) {
...
- (void)dynamic_downloadNextVideoSegment {
    ...
    __weak DashMultimediaManager *theWeakSelf = self;
    
    [self.segmentManager downloadVideoSegment:videSegmentURL
                          withCompletionBlock:^(BOOL success, NSError *error){
                              __strong DashMultimediaManager *theStrongSelf = theWeakSelf;
...
- (NSURL *)dynamic_nextVideoSegmentURL {
    ...
    pathComponent = [pathComponent stringByReplacingOccurrencesOfString:TemplateTime
                                                             withString:tString];
...
- (NSURL *)dynamic_nextAudioSegmentURL {
    ...
    template = [template stringByReplacingOccurrencesOfString:TemplateTime
                                                   withString:tString];
...
- (NSURL *)static_nextAudioSegmentURL {
    ...
    if ([template rangeOfString:TemplateRepresentationID].location != NSNotFound){
        template = [template stringByReplacingOccurrencesOfString:TemplateRepresentationID
                                                       withString:[self static_currentAudioRep].id];
    }
    
    if ([template rangeOfString:TemplateNumber].location != NSNotFound){
        template = [template stringByReplacingOccurrencesOfString:TemplateNumber
...
- (NSURL *)initialVideoSegmentUrlForRep:(Representation *)rep {
    ...
    NSRange range = [initSegUrl rangeOfString:TemplateRepresentationID];
    
    if (range.location != NSNotFound){
        NSString *link = [initSegUrl stringByReplacingOccurrencesOfString:TemplateRepresentationID
                                                               withString:rep.id];
