//
//  DashMultimediaManagerForRange.m
//  DASH Player
//
...
#import "DashMultimediaManagerForRange.h"
#import "HttpClient.h"
#import "MPD.h"
#import "MpdManager.h"
#import "MultimediaData/SegmentManager/SegmentManager.h"
#import "Sidx.h"
#import "VideoData.h"
#import "AudioData.h"

@interface DashMultimediaManagerForRange () <MpdManagerDelegate> {
...
@property (nonatomic, strong) MpdManager      *mpdManager;
@property (nonatomic, strong) SegmentsManager *segmentManager;
...
@property (nonatomic, strong) VideoData           *firstVideoData;
@property (nonatomic, strong) AudioData           *firstAudioData;
...
@implementation DashMultimediaManagerForRange
...
- (id)init {
    ...
    if (self){
        ...
        self.segmentManager                 = [[SegmentsManager alloc] init];
        self.firstStart                     = YES;
...
- (void)setMpdUrl:(NSURL *)mpdUrl {
    ...
    self.mpdManager = [[MpdManager alloc] initWithMpdUrl:mpdUrl];
...
#pragma mark - MpdManagerDelegate
...
- (BOOL)downloadInitialSegmentForRep:(Representation *)rep saveToDictionary:(NSMutableDictionary *)dictionary {
    ...
    __weak DashMultimediaManagerForRange *theWeakSelf = self;
...
- (BOOL)downloadFirstMediaSegments {
    ...
    NSUInteger offset_v     = [currentSegmentReference[SidxConstantOffset] unsignedIntegerValue];
    NSUInteger size_v       = [currentSegmentReference[SidxConstantSize] unsignedIntegerValue];
    NSUInteger timescale_v  = [currentSegmentReference[SidxConstantTimescale] unsignedIntegerValue];
    NSUInteger duration_v   = [currentSegmentReference[SidxConstantDuration] unsignedIntegerValue];
    NSUInteger time_v       = [currentSegmentReference[SidxConstantTime] unsignedIntegerValue];
    ...
    [self.segmentManager downloadData:baseUrl
                              atRange:range_v
                  withCompletionBlock:^(BOOL success, NSError *error, id response){
                      __strong DashMultimediaManagerForRange *theStrongSelf = theWeakSelf;
    ...
    NSUInteger offset_a     = [currentSegmentReference[SidxConstantOffset] unsignedIntegerValue];
    NSUInteger size_a       = [currentSegmentReference[SidxConstantSize] unsignedIntegerValue];
    NSUInteger timescale_a  = [currentSegmentReference[SidxConstantTimescale] unsignedIntegerValue];
    NSUInteger duration_a   = [currentSegmentReference[SidxConstantDuration] unsignedIntegerValue];
...
- (Representation *)selectVideoRepresentation {
    ...
    NSUInteger duration = [sidxInfo[SidxConstantDuration] integerValue];
    double timescale = (double)[sidxInfo[SidxConstantTimescale] integerValue]; //
...
- (void)calculateVideoSidxForPos:(NSTimeInterval)pos andActualVideoPos:(double *)actualPos {
    ...
    for (NSDictionary *dic in references){
        double time = [dic[SidxConstantTime] unsignedIntegerValue] / timescale;
        if (time >= pos){
            break;
        }
        index ++;
    ...
    *actualPos = [targetDic[SidxConstantTime] unsignedIntegerValue] / timescale;
}
...
- (void)calculateAudioSidxForPos:(NSTimeInterval)pos {
    ...
    for (NSDictionary *dic in references){
        double time = [dic[SidxConstantTime] unsignedIntegerValue] / (double)timescale;
        if (time >= pos){
