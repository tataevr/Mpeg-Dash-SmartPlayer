...
-(instancetype)parseSidx:(NSData *)sidxData withFirstByteOffset:(uint32_t)byteOffset {
    ...
    for (i = 0; i < self.referenceCount; i++) {
        ...
        pos += 12;
        NSDictionary *dic = @{SidxConstantSize        : @(ref_size),
                              SidxConstantType        : @(ref_type),
                              SidxConstantOffset      : @(offset),
                              SidxConstantDuration    : @(ref_dur),
                              SidxConstantTime        : @(time),
                              SidxConstantTimescale   : @(self.timescale)};
...
- (NSTimeInterval)scaledDurationForReferenceNumber:(NSUInteger)referenceNumber {
    if (referenceNumber < self.referenceCount){
        ...
        NSUInteger duration = [dic[SidxConstantDuration] unsignedIntegerValue];
        NSUInteger timescale = [dic[SidxConstantTimescale] unsignedIntegerValue];
