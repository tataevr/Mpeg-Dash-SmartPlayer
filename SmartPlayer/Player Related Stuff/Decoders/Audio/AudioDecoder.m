...
static NSUInteger const ReadBufferSize = 4096;
...
@property (nonatomic, strong) AudioData *audioData;
...
+ (void) __attribute__((noreturn)) audioDemuxThreadEntryPoint:(id)__unused object {
    do {
        @autoreleasepool {
            [[NSThread currentThread] setName:@"AudioDemuxThread"];
...
static int readFunction(void* opaque, uint8_t* buf, int buf_size) {
    AudioDecoder *afe = (__bridge AudioDecoder *)(opaque);
    if (afe->bytesLeft == 0) {
        return -1;
    }
...
- (id)initWithAudioData:(AudioData *)audioData {
    ...
    self.ringBuffer = [[RingBuffer alloc] initWitSize:AUDIO_BUF_SIZE
...
-(AVStream *) openAudioStream {
    ...
    // Find suitable codec
    NSLog(@"Searching suitable codec");
