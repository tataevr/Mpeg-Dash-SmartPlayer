...
static NSUInteger const ReadBufferSize = 8192;
...
+ (void) __attribute__((noreturn)) demuxThreadEntryPoint:(id)__unused object {
    do {
        @autoreleasepool {
            [[NSThread currentThread] setName:@"VideoDemuxThread"];
...
static int ReadFunction(void* opaque, uint8_t* buf, int buf_size) {
    VideoDecoder *vd = (__bridge VideoDecoder *)(opaque);
    if (vd->bytesLeft == 0) {
...
- (void)configSupportingObjects {
    ...
    self.videoPicturesList  = [[VideoPicturesList alloc] init];
}
...
- (BOOL)openVideo {
    ...
    // Open video file
    Byte *buffer = av_malloc(ReadBufferSize);
...
- (VideoData *)nextVideoData {
    VideoData *object = [_videoDataArray firstObject];
...
#pragma mark - setters
- (void)addNextVideoData:(VideoData *)videoData {
    ...
    while(av_read_frame(formatContex, &thePacket) >= 0 && !self->quit) {
        if(thePacket.stream_index == videoStream) {
            ...
            if (gotFrame){
                ...
                VideoFrameYUV *frame = [self handleVideoFrame:pFrame];
                PicturesListElement *element = [[PicturesListElement alloc] init];
...
- (VideoFrameYUV *) handleVideoFrame:(AVFrame *)frame {
    if (!frame->data[0]){
        return nil;
    }
